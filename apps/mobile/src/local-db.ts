...
export async function listRecords() {
  return db.getAllAsync<LocalRecord>(
    "SELECT id, title, description, latitude, longitude, photo_uri as photoUri, captured_at as capturedAt, sync_status as syncStatus FROM field_records ORDER BY captured_at DESC",
  );
}

export async function saveRecord(record: LocalRecord) {
  await db.runAsync(
    "INSERT INTO field_records (id, title, description, latitude, longitude, photo_uri, captured_at, sync_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    record.id,
    record.title,
    record.description,
    record.latitude,
...
export async function createLocalRecord(
  record: Omit<
    LocalRecord,
    "description" | "syncStatus" | "latitude" | "longitude"
